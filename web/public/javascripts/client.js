	...
	$("#playlist").load(url, function(data) {
		$("#playlist ul").show().sortable({
			...
			update: function(e, ui) {
				...
				$.post("/playlist/" + current_playlist + "/sort", {
					video: $(ui.item).attr("id"),
					pos: position
				},
				function() {
...
function delete_video(id) {
	$.ajax({
		type: 'DELETE',
		url: '/playlist/' + current_playlist + '/videos/' + id,
...
$.widget("ui.playlists_manager", {
	_init: function() {
		...
		$el.find(".add-playlist a").click(function(e) {
			...
			e.preventDefault();
			var playlist = {
				title: $el.find(".add-playlist input").val()
			};
...
jQuery(document).ready(function($) {     
  ...
  // });
	...
	$("#add-video input").addClass("empty").val(default_video_input_text).focus(function() {
		$(this).val("");
	...
	$("#add-playlist-btn a").click(function(e) {
		...
		$layout.removeClass("layout").addClass("new").show().find(".title").hide();
		var $input = $("<input/>");
		$("#add-playlist-btn").addClass("on");
		$layout.find("a.info").append($input.attr("type", "text"));
		$("#playlists ul").append($layout);
		$layout.find("input").focus().end().find(".edit").show().find(".cancel").click(function(e) {
	...
	$(".playlist-item .delete").live("click", function(e) {
		...
		if (x) delete_playlist($item.attr("id"));
	});
	...
	$("#playlist_published").live('change', function() {
		$.post("/playlist/" + current_playlist, {
